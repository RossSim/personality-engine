...
Read docs/CHARTER.md first. For how games should *use* the engine at design time (not the C# API), read docs/APPLICATIONS.md. OCEAN, PAD, and OCC in ordinary language: docs/OCEAN_PAD_OCC.md. A game that uses a language model hosts PE the same way; the model stays outside: docs/LANGUAGE_MODELS.md. Public version direction: docs/ROADMAP.md (includes later provider themes and the separate Archetypes companion at https://github.com/RossSim/archetypes). The engine is modular middleware: composable, citable providers. OCEAN, PAD, and OCC are the default composition (Gebhard ALMA 2005 wiring), not the ceiling. Hosts may add layers, replace a layer’s provider, or supplement a layer with more sources/methods. Preset catalogs (profession, temperament, fantasy clan) live in Archetypes, not here. No LLM in the core. Current published library is 0.6.2 (plain-language front door, diagrams, playable examples host; GitHub Packages).
