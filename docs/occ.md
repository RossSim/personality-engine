...
| --- | --- | --- | --- |
| `OccEmotion` (`occ`) | emotion | 20 OCC types as `emotion.occ.*` channels | Ortony, Clore & Collins (1988) |
...
| Well-being + attribution | gratification, gratitude, anger, remorse | `occ.gratification`, `occ.gratitude`, `occ.anger`, `occ.remorse` |

The host still tags the eliciting condition. Compounds are **their own types**: tagging `occ.gratification` does not also write joy or pride. `WorldEvent.Target` may name the blameworthy or praiseworthy other for anger and gratitude; snapshot channels stay **global** (`emotion.occ.anger`), not per-agent keys. Pairwise liking belongs on the optional [`dyad.md`](dyad.md) relationship provider. Do not infer anger from dislike or from distress+reproach.

Not in this slice: attraction (love, hate), inferring fortune-of-others or compounds from untyped events or from liking, and automatic appraisal from untyped `WorldEvent`s.
